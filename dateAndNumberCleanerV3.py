...
originalTextDocsPath = ['C:/Users/bat/Desktop/test_data']
...
pathForInputFoldersForChronos = 'C:/Users/bat/Desktop/testFile'

#### users provides input for the location of Chronos 
#print('Please Provide the Path for the Chronos Tool')
chronosLocation = 'C:/Users/bat/Anaconda3/Lib/site-packages/Chrono/Chrono.py'
...
for inputFile1 in inputTextFilePathList: 
    ...
    numberExtra2 = re.sub(r'\<NUM|\>NUM|NUM\smg|NUMmg|NUML|NUM\sL|NUMml|NUMml\.|NUMC\.|NUMC|NUMF\.|NUMF|NUMmg\.|\%NUM|\^NUM|\*NUM|\+NUM|\-NUM|NUM\s+NUM\s+NUM|NUM\s+NUM|\=NUM|\#NUM|\/NUM|\~NUM|\`NUM|\"NUM|\'NUM','NUM',numberExtra1)
    
    numberExtra3 = re.sub(r'NUM\s+\,\s+NUM\s+\,\s+NUM\s+\,\s+NUM|NUM\s+\,\s+NUM\s+\,\s+NUM|NUM\s+\,\s+NUM','NUM',numberExtra2)
    
    numberExtra3A = re.sub(r'NUMPM|NUM\s+PM|NUM\s+AM|NUM\:NUM|NUMAM|NUM\:NUMPM','NUM',numberExtra3)
    
    numberExtra3B = re.sub(r'\w+NUM\w+|\w+NUM|NUM\w+|NUMPM|NUMAM|NUM\s+PM|NUM\s+AM','NUM',numberExtra3A)
    
    numberExtra4 = re.sub(r'NUMNUMNUM|NUMNUM|NUM\s+NUM\s+NUM|NUM\s+NUM', 'NUM', numberExtra3B)
